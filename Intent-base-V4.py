...
class JointModelWithEarlyExit(nn.Module):
    def __init__(self, args, num_intent, num_slot):
        ...
        ratio = max(0.5, min(1.0, getattr(args, "unfrozen_ratio", 0.5)))
        ...
        self.num_unfrozen_layers = max(1, math.ceil(total_bb_layers * ratio))
    ...
    def _apply_partial_freeze(self):
        ...
        first `self.num_unfrozen_layers` transformer blocks only (embeddings
        and any deeper blocks stay frozen). If the backbone doesn't expose a
        `.layers` list (same architecture requirement as `_true_layer_iter`
        elsewhere in this file), falls back to the original fully-frozen
        behaviour with a warning rather than guessing at a different API.
        """
        ...
        if hasattr(bm, "layers"):
            for i, layer in enumerate(bm.layers):
                if i < self.num_unfrozen_layers:
                    for p in layer.parameters():
                        p.requires_grad_(True)
            logger.info(
                "Partial backbone unfreeze: layers[0:%d] TRAINABLE, "
                "layers[%d:%d] FROZEN (embeddings frozen too). "
                "unfrozen_ratio=%.2f (clamped to >= 0.5).",
                self.num_unfrozen_layers, self.num_unfrozen_layers,
                len(bm.layers), getattr(self.args, "unfrozen_ratio", 0.5),
            )
        else:
            logger.warning(
                "Backbone does not expose .layers -- cannot selectively "
                "unfreeze early layers. Falling back to a FULLY FROZEN "
    ...
    def train(self, mode: bool = True):
        ...
        if mode and self.num_unfrozen_layers > 0 and hasattr(bm, "layers"):
            bm.train(True)
            for i, layer in enumerate(bm.layers):
                layer.train(i < self.num_unfrozen_layers)
        else:
            bm.eval()
        return self
